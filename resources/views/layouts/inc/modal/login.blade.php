				...
				<div class="modal-body">
					...
					<div class="form-group <?php echo (isset($errors) and $errors->has('password')) ? 'has-error' : ''; ?>">
						...
						<div class="input-icon"><i class="icon-lock fa"></i>
							<input id="mPassword" name="password" type="password" class="form-control" placeholder="{{ t('Password') }}">
						</div>
					</div>
					...
					<div class="form-group <?php echo (isset($errors) and $errors->has('remember')) ? 'has-error' : ''; ?>">
						...
						<p class="pull-right" style="margin-top: 10px;">
							<a href="{{ lurl(trans('routes.password-reset')) }}" rel="nofollow"> {{ t('Lost your password?') }} </a> / <a href="{{ lurl(trans('routes.register')) }}" rel="nofollow">{{ t('Register') }}</a>
						</p>
