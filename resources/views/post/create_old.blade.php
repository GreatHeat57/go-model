										...
										<div class="form-group required <?php echo (isset($errors) and $errors->has('ismodel')) ? 'has-error' : ''; ?>">
											...
											<div class="col-md-9">
												<label class="radio-inline" for="ismodel">
													<input name="ismodel" value="1" type="radio" @if (old('ismodel')=='' or old('ismodel')==1)
																checked="checked"
															@endif required="required">
													{{ t('Model') }}
												</label>
												<label class="radio-inline" for="ismodel">
													<input name="ismodel" value="0" type="radio" required="required" @if (old('ismodel')==0)
																checked="checked"
															@endif>
													{{ t('Partner') }}
												</label>
										...
										<div class="form-group required <?php echo (isset($errors) and $errors->has('gender_type')) ? 'has-error' : ''; ?>">
											...
											<div class="col-sm-8">
												<select name="gender_type" id="userType" class="form-control selecter">
													...
													<option value="9">{{ t("Male") }} </option>
													<option value="10">{{ t("Female") }} </option>
...
@section('after_scripts')
    ...
    <script type='text/javascript' src="{{ URL::asset('js/bootstrap-slider.js')}}"></script>
        ...
        <script type="text/javascript">
			// body height
			$("#height").slider({});
			var slider = new Slider('#height', {});

			// body weight
			$("#weight").slider({});
			var slider = new Slider('#weight', {});

			$("#age").slider({});
			var slider = new Slider('#age', {});

			$("#dress_size").slider({});
			var slider = new Slider('#dress_size', {});
