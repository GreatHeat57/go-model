...
<tr style="font-family: 'Helvetica Neue', 'Helvetica', Helvetica, Arial, sans-serif; font-size: 100%; line-height: 1.6em; margin: 0; padding: 0;">
    ...
    <table style="font-family: 'Helvetica Neue', 'Helvetica', Helvetica, Arial, sans-serif; font-size: 100%; line-height: 1.6em; width: 100%; margin: 0; padding: 0;">
		...
		<h2>@lang('mail.Hello') {{ $data['partner_name'] }},</h2>

		<p>@lang('mail.congratulation'), {{ $data['model_name']}} @lang('mail.accepted_your_Invitation')</p>    
            @php $messageid = $data['messageid'];
                $model_id = $data['model_id'];
            @endphp
        <p><a href="{{ url('/account/conversations/'.$messageid.'/messages') }}">@lang('mail.view message')</a> </p>
        <p><a href="{{ lurl(trans('routes.user').'/'.$data['username']) }}">@lang('mail.view_model_profile')</a></p>
