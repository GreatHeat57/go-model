		...
		<div class="container">
			<div class="row">
				...
				<div class="col-sm-9 page-content">
                    ...
                    <div class="inner-box">
						<h2 class="title-2"><i class="icon-trophy"></i> {{ t('Edit achievements / references') }} </h2>
									...
									<div class="panel-body">
										<form name="details" class="form-horizontal" role="form" method="POST" action="{{ lurl('account/profile/reference') }}" enctype="multipart/form-data">
											...
											<div class="form-group required <?php echo (isset($errors) and $errors->has('reference.title')) ? 'has-error' : ''; ?>">
												...
												<div class="col-sm-8">
													<input name="title" type="text" class="form-control" placeholder="{{ t('Title') }}" value="{{ old('name', $reference['title']) }}">
												</div>
											</div>
											...
											<div class="form-group required <?php echo (isset($errors) and $errors->has('reference.description')) ? 'has-error' : ''; ?>">
												...
												<div class="col-sm-8">
													<textarea id="reference_description" name="description">{{ $reference['description'] }}</textarea>
												</div>
											</div>
											...
											<div class="form-group">
												<div class="col-sm-offset-5 col-sm-7">
													<button name="update" type="submit" class="btn btn-primary">{{ t('Submit') }}</button>
												</div>
											</div>
	...
	<script>
		...
		function set_date($date){
			if ( $date != '' ) {
				...
				$('[name="reference_date_birthDay"]').val($date);
			}
		}

		set_date('<?php echo $reference['date'] ?>');
	</script>
@endsection
