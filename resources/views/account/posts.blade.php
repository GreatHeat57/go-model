											...
											<div>
												...
												<p>
													...
													@if (file_exists(public_path($iconPath)))
														<img src="{{ url($iconPath) }}" data-toggle="tooltip" title="{{ $post->country_code }}" alt="{{ strtolower($post->country_code).'.png' }}">
													@endif
													<?php $applications = \App\Models\Message::where('post_id', $post->id)->where('to_user_id', auth()->user()->id)->groupBy('from_user_id')->get();?>
													<a href="{{lurl('account/my-posts/'.$post->id.'/applications')}}" style="float: right;">{{ count($applications) }} {{ t('applications') }}</a>
												</p>
											</div>
										</td>
											...
											<div>
												@if ($post->user_id==$user->id and $post->archived==0)
													<p>
                                                        ...
                                                        <a class="btn btn-primary btn-sm" href="{{ lurl(trans('routes.post-edit', $attrId), $attrId) }}">
