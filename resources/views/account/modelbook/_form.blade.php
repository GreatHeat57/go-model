	...
	@if ($originForm != 'message')
			...
			<div class="form-group <?php echo (isset($errors) and $errors->has('modelbook.name')) ? 'has-error' : ''; ?>">
				...
				<div class="{{ $classRightCol }}">
					<input name="modelbook[name]"
						   placeholder="{{ t('Name') }}"
						   class="form-control input-md"
						   type="text"
						   value="{{ old('modelbook.name', (isset($modelbook->name) ? $modelbook->name : '')) }}">
				</div>
			</div>
		...
		<div class="form-group <?php echo (isset($errors) and $errors->has('modelbook.filename')) ? 'has-error' : ''; ?>">
			<label class="{{ $classLeftCol }} control-label" for="modelbook.filename"> {{ t('Your model book') }} </label>
			<div class="{{ $classRightCol }}">
				<div class="mb10">
				...
				@if (isset($modelbook))
					@if (!empty($modelbook) and !empty($modelbook->filename) and \Storage::exists($modelbook->filename))
					<div>
						<img src="{{ \Storage::url($modelbook->filename) }}" height="100px" width="100px">
						<a class="btn btn-default" href="{{ \Storage::url($modelbook->filename) }}" target="_blank" download="Sedcard" onclick="window.event.ctrlKey">
