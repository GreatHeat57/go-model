		...
		<div class="container">
			<div class="row">
				...
				<div class="col-sm-9 page-content">
					<div class="inner-box">
						<h2 class="title-2"><i class="icon-town-hall"></i> {{ t('Picture album') }} </h2>
						<div class="mb30">
							<a href="{{ lurl('account/albem/create') }}" class="btn btn-default"><i class="icon-plus"></i> {{ t('Add a new picture') }}</a>
						</div>
						<br>
										...
										<td style="width:10%" class="action-td">
											<div>
												@if ($modelbook->user_id==$user->id)
													<p>
														<a class="btn btn-primary btn-sm" href="{{ lurl('account/album/' . $modelbook->id . '/edit') }}">
															<i class="fa fa-edit"></i> {{ t('Edit') }}
														</a>
													</p>
													<p>
														<a class="btn btn-danger btn-sm delete-action" href="{{ lurl('account/album/'.$modelbook->id.'/delete') }}">
															<i class="fa fa-trash"></i> {{ t('Delete') }}
														</a>
													</p>
												@endif
					...
					<?php if(isset($data) AND $data->count() > 0){ ?>
						...
						<div class="genrate-sedcard">
							<a class="btn btn-primary" href="{{ lurl('account/album/genrate/') }}/<?php echo $user->id; ?>">
								{{ t('Photo portfolio') }}
							</a>
