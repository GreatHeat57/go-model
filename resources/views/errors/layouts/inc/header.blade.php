			...
			<div class="container">
				<div class="navbar-header">
					...
					<a href="{{ url(config('app.locale') . '/') }}" class="navbar-brand logo logo-title">
						<img src="{{ \Storage::url(config('settings.app.logo')) . getPictureVersion() }}" style="width:auto; height:40px; float:left; margin:0 5px 0 0;"/>
					</a>
				</div>
					...
					<ul class="nav navbar-nav navbar-left">
						...
						@if (getSegment(1) != trans('routes.countries'))
							@if (config('settings.geo_location.country_flag_activation'))
								@if (!empty(config('country.icode')))
									@if (file_exists(public_path().'/images/flags/32/'.strtolower(config('country.icode')).'.png'))
							...
							<li class="dropdown">
								...
								<ul class="dropdown-menu user-menu">
									...
									@if (in_array(auth()->user()->user_type_id, [1, 2]))
										...
										<li><a href="{{ url(config('app.locale') . '/account/conversations') }}"><i class="icon-mail-1"></i> {{ t('Message') }} </a></li>
