			...
			<div class="row">
									...
									@foreach($pages as $page)
										<li>
											...
											@if (!empty($page->external_link))
												<a href="{!! $page->external_link !!}" {!! $linkTarget !!}> {{ $page->name }} </a>
											@else
				...
				<div class="col-lg-2 col-md-2 col-sm-2 col-xs-12">
					<div class="footer-col">
						...
						<ul class="list-unstyled footer-nav">
							@if (!auth()->user())
								<li><a href="{{ lurl(trans('routes.login')) }}" rel="nofollow"> {{ t('Log In') }} </a></li>
								<li><a href="{{ lurl(trans('routes.register')) }}" rel="nofollow"> {{ t('Register') }} </a></li>
							@else
								<li><a href="{{ lurl('account') }}"> {{ t('Personal Home') }} </a></li>
								@if (isset($user))
									@if (in_array($user->user_type_id, [1, 2]))
										<li><a href="{{ lurl('account/my-posts') }}"> {{ t('My ads') }} </a></li>
										<li><a href="{{ lurl(trans('routes.account-companies')) }}"> {{ t('My companies') }} </a></li>
									@endif
