...
trait Crud
{
    ...
    */
	...
	public function translations()
	{
        ...
        if(class_basename($model) == "Post"){
            ...
            $lang1 = config('appLang.abbr');
            return $model::select('posts.*', 'jobs_translation.description', 'jobs_translation.title', 'jobs_translation.job_id', 'jobs_translation.translation_lang', 'jobs_translation.id as jobs_translation_id')
                        ->leftJoin('jobs_translation', function ($join) use ($lang1){
                            $join->on('jobs_translation.job_id', '=' , 'posts.id');
                            $join->where('jobs_translation.translation_lang', '!=', $lang1);
                        })
                        ->where('jobs_translation.job_id', $this->id)
                        ->get(); 
        }
	...
	public function save(array $options = [])
	{
        ...
        // if post update unset object translation_lang
        if(class_basename($this) == "Post"){
            
            unset($this->translation_lang);
        }
    ...
    */
	...
	public function uploadFileToDiskCustom($value, $field_name, $attribute_name, $disk, $destination_path, $user_id = null, $is_thumb = false, $thumb_size = [] )
	{
		...
		if ($request->hasFile($field_name) &&
            ...
            // temp code disable to create thumb

            // if($is_thumb && $user_id){
                    
            //     foreach ($thumb_size as $key => $size) {
            //         $thumb_image = str_replace($user_id.'/', $user_id.DIRECTORY_SEPARATOR.$thumb.DIRECTORY_SEPARATOR.$size.'_', $this->{$attribute_name});
            //         Storage::disk($disk)->delete($thumb_image);
            //     }
            // }
		...
		if ($request->hasFile($field_name) && $request->file($field_name)->isValid()) {
            ...
            // if thumb image is required then create new image

            // temp code disable to create thumb
            /*if($is_thumb && $user_id){
                
                $thumbnailPath = public_path($disk).DIRECTORY_SEPARATOR.$destination_path.DIRECTORY_SEPARATOR;

                $path = str_replace('/', DIRECTORY_SEPARATOR, $thumbnailPath.$thumb);

                if (!file_exists($path)) {
                    mkdir($path, 0777, true);
                }
                
                if( !empty($thumb_size) && count($thumb_size) > 0 ){
                    
                    $thumb_obj = array();

                    foreach ($thumb_size as $key => $size) {

                        $thumb_obj[$key] = Image::make($file);
                        $thumb_obj[$key]->resize($size, $size);
                        $thumb_obj[$key]->save($path.DIRECTORY_SEPARATOR.$size.'_'.$new_file_name);
                    }
                }
            }*/
