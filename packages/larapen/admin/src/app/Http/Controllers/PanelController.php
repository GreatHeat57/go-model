...
class PanelController extends Controller {
	use AjaxTable, Reorder, ShowDetailsRow, SaveActions, TranslateItem;
	...
	public function storeCrud(StoreRequest $request = null) {
		try {
			...
			foreach ($request->input() as $key => $value) {
				...
				//check if request from Blog Entry then update content
				if(isset($this->xPanel->entity_name) && $this->xPanel->entity_name === 'Blog Entry'){
					$newVal = $this->xPanel->model->removeBRTagFromImage($key, $value, 'long_text');
					$request->request->set($key, $newVal);
				}

				//check if request from Blog Entry then update content
				if(isset($this->xPanel->entity_name) && $this->xPanel->entity_name === 'Page'){
					$BlogEntry = new BlogEntry();
					$newVal = $BlogEntry->removeBRTagFromImage($key, $value, 'content');
					$request->request->set($key, $newVal);
				}
			}
	...
	public function edit($id, $lang = 'en', $childId = null) {
		...
		// only for use get post edit page
		if(!empty($lang)){

			\Config::set('app.post_locale', $lang); 
		}
