...
trait TranslateItem
{
    ...
    public function translateItem($parentId, $id = null, $lang = null)
    {
        ...
        // if check curent model is post
        if(class_basename($this->xPanel->model) == 'Post'){

            $getJobsTranslation = JobsTranslation::where('job_id', $id)->where('translation_lang', 'en')->first();
            
            $jobsTranslation = new JobsTranslation();
            $jobsTranslation->job_id = $id;
            $jobsTranslation->translation_lang = $lang;

            if(!empty($getJobsTranslation)){
                $jobsTranslation->title = $getJobsTranslation->title;
                $jobsTranslation->description = $getJobsTranslation->description;
            }else{
                $jobsTranslation->job_id = $id;
                $jobsTranslation->translation_lang = $lang;
            }
            ...
            return redirect(str_replace($id . '/', $id . '/' .$lang.'/', str_replace('translate/' . $lang, 'edit', \Request::url())));  
        }
