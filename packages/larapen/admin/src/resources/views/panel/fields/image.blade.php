<div data-preview="#{{ $field['name'] }}" data-aspectRatio="{{ isset($field['aspect_ratio']) ? $field['aspect_ratio'] : 0 }}" data-crop="{{ isset($field['crop']) ? $field['crop'] : false }}" @include('admin::panel.inc.field_wrapper_attributes')>
    ...
    <div class="row">
        <div class="col-sm-6" style="margin-bottom: 20px;">
        	<img id="mainImage" src="{{ url( (isset($field['prefix']) ? $field['prefix'] : '') . (old($field['name']) ? old($field['name']) : (isset($field['value']) ? \Storage::disk($field['disk'])->url($field['value']) : (isset($field['default']) ? $field['default'] : 'images/user.jpg') ))) }}">
        </div>
...
@if ($xPanel->checkIfFieldIsFirstOfItsType($field, $fields))
    ...
    {{-- YOUR CSS HERE --}}
    <link href="{{ asset('vendor-admin/vendor-admin/admin/cropper/dist/cropper.min.css') }}" rel="stylesheet" type="text/css" />
    <style>
        .hide {
            display: none;
        }
    ...
    {{-- YOUR JS HERE --}}
    <script src="{{ asset('vendor-admin/vendor-admin/admin/cropper/dist/cropper.min.js') }}"></script>
    <script>
