                        ...
                        <tr>
                            ...
                            @foreach ($xPanel->columns as $column)
                                @if (isset($column['type']) && $column['type']=='select_multiple')
                                    ...
                                    <td><?php
                                        ...
                                        ?></td>
                                @elseif (isset($column['type']) && $column['type']=='select' && $modelname != 'Post')
                            ...
                            @if ( $xPanel->hasAccess('update') or $xPanel->hasAccess('delete') )
                                ...
                                <?php
                                    ...
                                    $editUrl = str_replace('/'.$original_entry->id, '/'.$entry->id, str_replace('details', 'edit', Request::url()));

                                    $deleteUrl = str_replace('/'.$original_entry->id, '/'.$entry->id, str_replace('details', '', Request::url()));

                                    // post 
                                    if(isset($modelname) && $modelname == 'Post'){

                                        $editUrl = str_replace('/'.$original_entry->id, '/'.$entry->id.'/'.$entry->translation_lang, str_replace('details', 'edit', Request::url()));
                                    }
                                ?>
                                <td>
                                    ...
                                    @if ($xPanel->hasAccess('delete'))
                                        <a href="{{ $deleteUrl }}" class="btn btn-xs btn-default" data-button-type="delete">
