		...
		if (!empty($value)) {
			...
			if ($this->key == 'app') {
				...
				if (!isset($value['purchase_code'])) {
					...
					$value['purchase_code'] = config('app.purchase_code');
				}
				if (!isset($value['name'])) {
			...
			if ($this->key == 'upload') {
				...
				if (!isset($value['resume_types'])) {
					$value['resume_types'] = 'pdf,doc,docx';
				}
	...
	*/
	...
	private function upload($value, $params) {
		...
		if (starts_with($value[$attribute_name], 'data:image')) {
			...
			if (!empty($params['default'])) {
				$isDefaultFile = Str::contains($value[$attribute_name], $params['default']);
			} else {
