...
class Handler extends ExceptionHandler
{
	...
	public function render($request, Exception $e)
	{
		...
		//send exceptions email to admin "admin@itellico.com"
		if ($e instanceof \Exception) {

	        /*Mail::send('emails.exception', ['error' => $e->getMessage(), 'url' => Request::url(), 'ErrorCode' => $ErrorCode], function ($m) {
	            $m->to(config('app.admin_email'), 'admin')->bcc(config('app.admin_email2'), 'admin2')->subject( trans('mail.error_subject') );
	        });*/
		...
		if ($this->isHttpException($e)) {
			...
			//switch (intval($e->getStatusCode())) {
	            // not found
	            // case 404:
	            // 	\Log::error('Error in request url', ['Request url get 404 error' => $request->fullUrl()]);
	...
	{
		// Database Parameters
		$driver = Config::get('database.connections.' . Config::get('database.default') . '.driver', 'mysql');
		$host = Config::get('database.connections.' . Config::get('database.default') . '.host');
		$port = (int)Config::get('database.connections.' . Config::get('database.default') . '.port');
		$socket = Config::get('database.connections.' . Config::get('database.default') . '.unix_socket');
		...
		try {
			...
			$msg = "<pre>Can't connect to the database server. ERROR: <strong>" . $e->getMessage() . "</strong></pre>";
			return $msg;
		} catch (\Exception $e) {
			$msg = "<pre>The database connection failed. ERROR: <strong>" . $e->getMessage() . "</strong></pre>";
			return $msg;
		}
	...
	private function getLanguage()
	{
		...
		if ($this->isInstalled()) {
			try {
				...
				$lang = Cache::remember($cacheId, 1440 * 60, function () {
