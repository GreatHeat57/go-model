...
class LocationController extends FrontController
{
    ...
    {
        $query = RequestFacade::input('query');
    ...
    public function getAdmins($countryCode, $adminType)
    {
        ...
        $admins = Cache::remember($cacheId, $this->cacheExpiration, function () use ($model, $countryCode, $adminType) {
			if ($adminType == 2) {
				$admins = $model::countryOf($countryCode)->orderBy('name')->get(['code', 'name', 'subadmin1_code']);
			} else {
        ...
        if ($admins->count() == 0) {
            return response()->json([
                'error' => [
                    'message' => t("No admin. division doesn't exists for the current country.", [], 'global', RequestFacade::input('languageCode'))
                ], 404
            ]);
        }
        ...
        if ($adminType == 2) {
            ...
            foreach($admins as $admin) {
				$admin1Code = '';
				if (isset($admin->subadmin1_code) && !empty($admin->subadmin1_code)) {
					$admin1Code = ', ' . preg_replace('/^' . $countryCode . '\./i', '', $admin->subadmin1_code);
				}
                $tab[] = [
                    'code' => $admin->code,
                    'name' => $admin->name . $admin1Code,
                ];
            }
            $admins = $tab;
        }
    ...
    public function getSelectedCity($countryCode, $cityId)
    {
        ...
        if (!empty($city)) {
            ...
            $city = ['id' => 0, 'text' => t('Select a city', [], 'global', RequestFacade::input('languageCode'))];
        }
    ...
    public function getCityByCountry($countryCode)
    {
        ...
        if(isset($countryCode) && !empty($countryCode)){
            ...
            $citiess = City::select('id','name')->where('country_code', $countryCode)->orderby('name','asc')->get()->toArray();
        }

        $status = ['success' => 'false', 'data' => null];

        if( count($citiess) > 0 ){
            $status = ['success' => 'true', 'data' => $citiess];
        }
    ...
    /**
     * Form Select Box from country return city list
     * Get the selected City
     *
     * @param $countryCode
     * @param $cityId
     * @return \Illuminate\Http\JsonResponse
     */
    public function getCodeCities($countryCode, $citybyname = false){


        if( isset($countryCode) && !empty($countryCode) ){
            $cities = City::countryOf($countryCode)->orderby('name','ASC')->get();
