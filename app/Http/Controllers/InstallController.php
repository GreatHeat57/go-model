...
class InstallController extends Controller
{
		...
		if ($request->isMethod('post')) {
			...
			try {
				...
				if (empty($request->socket)) {
					$dsn = $driver . ':host=' . $request->hostname . ';port=' . $port . ';dbname=' . $request->database_name . ';charset=utf8';
				} else {
					$dsn = $driver . ':unix_socket=' . $request->socket . ';dbname=' . $request->database_name . ';charset=utf8';
				}
	...
	public function databaseImport(Request $request)
	{
		...
		if ($request->action == 'import') {
			...
			// Get PDO connexion
			$pdo = $this->getPDOConnexion($database);
			...
			$this->importDatabase($pdo, $database['tables_prefix'], $siteInfo);
			
			// Close PDO connexion
			$this->closePDOConnexion($pdo);
	...
	private function writeEnv($request)
	{
		...
		$content .= 'APP_FALLBACK_LOCALE=en' . "\n";
		...
		$content .= "\n";
		$content .= 'DB_HOST=' . (isset($database['hostname']) ? $database['hostname'] : '') . "\n";
		$content .= 'DB_PORT=' . (isset($database['port']) ? $database['port'] : '') . "\n";
		$content .= 'DB_DATABASE=' . (isset($database['database_name']) ? $database['database_name'] : '') . "\n";
		...
		$content .= 'DB_TABLES_PREFIX=' . (isset($database['tables_prefix']) ? $database['tables_prefix'] : '') . "\n";
		...
		$content .= 'APP_LOG=daily' . "\n";
		$content .= 'APP_LOG_MAX_FILES=2' . "\n";
		$content .= "\n";
	...
	private function updateDatabase($pdo, $tablesPrefix, $siteInfo)
	{
		...
		try {
			...
			$sql = 'INSERT INTO `' . $tablesPrefix . "users`
				(`id`, `country_code`, `user_type_id`, `gender_id`, `name`, `email`, `password`, `is_admin`, `verified_email`, `verified_phone`)
			...
			$res = $query->execute([
				...
				':password'    => bcrypt($siteInfo['password']),
			]);
	 ...
	 * @return \PDO
	 */
	private function getPDOConnexion($database)
	{
		// Database Parameters
		$driver = config('database.connections.' . config('database.default') . '.driver', 'mysql');
		$host = isset($database['hostname']) ? $database['hostname'] : '';
		...
		$database = isset($database['database_name']) ? $database['database_name'] : '';
	 ...
	 * @param $pdo
	 */
	private function closePDOConnexion(&$pdo)
	{
		$pdo = null;
	}
	...
	private function getDatabaseTables($pdo, $database)
	{
		...
		$prefixChecker = !empty($database['tables_prefix']) ? '  AND table_name LIKE "' . $database['tables_prefix'] . '%"' : '';
		$sql = 'SELECT GROUP_CONCAT(table_name) AS table_names
				FROM information_schema.tables
                WHERE table_schema = "' . $database['database_name'] . '"' . $prefixChecker;
		$query = $pdo->query($sql);
