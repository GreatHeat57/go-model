...
class BaseController extends FrontController
{
    ...
    public function commonQueries()
    {
        ...
        // Get all Branches
        $cacheId = 'branches.all.' . config('app.locale');
        $cats = Cache::remember($cacheId, $this->cacheExpiration, function () {
            $cats = Branch::trans()->orderBy('lft')->get();
            return $cats;
        });
        if ($cats->count() > 0) {
        ...
        // LEFT MENU VARS
        // Count Branches Posts
        /*$sql = 'SELECT sc.id, c.parent_id, count(*) as total' . '
            FROM ' . table('posts') . ' as a
            INNER JOIN ' . table('branches') . ' as sc ON sc.id=a.category_id AND sc.active=1
            INNER JOIN ' . table('branches') . ' as c ON c.id=sc.parent_id AND c.active=1
        ...
        // Count Parent Branches Posts
        /*$sql1 = 'SELECT c.id as id, count(*) as total' . '
            FROM ' . table('posts') . ' as a
            INNER JOIN ' . table('branches') . ' as c ON c.id=a.category_id AND c.active=1
            WHERE a.country_code = :countryCode AND (a.verified_email=1 AND a.verified_phone=1) AND a.archived!=1 AND a.deleted_at IS NULL
            GROUP BY c.id';
