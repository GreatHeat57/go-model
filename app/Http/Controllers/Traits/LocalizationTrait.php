...
trait LocalizationTrait {
	...
	private function loadLocalizationData() {
		...
		$activeLanguage = $langObj->supportedLanguage()->toArray();
		$supportedLanguage = array(); 
		if( isset($activeLanguage) && count($activeLanguage) > 0 ){
			foreach ($activeLanguage as $key => $language) {
				$supportedLanguage[] = $language['abbr'];
 			}

 			if($supportedLanguage && count($supportedLanguage) > 0 ){
 				Config::set('lang.supportedLanguage', $supportedLanguage);
 			}
		}
		...
		if (!$country->isEmpty() && $country->has('code')) {
			...
			Config::set('country.facebook_link', $country->get('facebook_link'));
			Config::set('country.instagram_link', $country->get('instagram_link'));
			Config::set('country.twitter_link', $country->get('twitter_link'));
			Config::set('country.pinterest_link', $country->get('pinterest_link'));
			Config::set('country.phone_code', $country->get('phone'));
		}

		if($country->has('feature_fallback_countries')){
			Config::set('country.feature_fallback_countries', $country->get('feature_fallback_countries'));
		}
		...
		// UK => reflang="en-GB", DE => hreflang="de-DE", EN => hreflang="en-US"
		// Config : reflang
	}
}
