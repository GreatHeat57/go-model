...
class PostRequest extends Request {
	...
	public function rules() {
		...
		if($this->input('is_date_announce') != 1){
			...
			$rules['start_date'] = 'required|date|after:yesterday';
		...
		if ($this->filled('ismodel') && $this->input('ismodel') == 1) {
			...
			$rules['age_from'] = 'required';
			$rules['age_to'] = 'required';
		...
		if (!$this->filled('company_id') || empty($this->input('company_id')) || $this->input('company_id') == 0) {
			...
			if ($this->has('company.name')) {
				$rules['company.name'] = 'required|mb_between:2,200|whitelist_word_title';
			}
			// Check 'description' is required
			if ($this->has('company.description')) {
				$rules['company.description'] = 'required|mb_between:100,1000|whitelist_word';
			}
		...
		*/
		...
		if($this->filled('weight_from') && $this->input('weight_from') > 0){
			$rules['weight_to'] = 'greater_than:weight_from|not_in:0|required';
		}

		if($this->filled('height_from') && $this->input('height_from') > 0){
		...
		if( $this->filled('age_from') && $this->filled('age_to') ){
			...
			$rules['age_to'] = 'greater_than:age_from|required';
		}
		...
		// }
	 	...
	 	// echo "<pre>";  print_r($rules);  "</pre>"; exit();
		return $rules;
	}
	...
	public function messages() {
		...
		$messages = [
			'company.name.required' => trans('validation.required'),
			'company.name.mb_between' => trans('validation.mb_between'),
			'company.name.whitelist_word_title' => trans('validation.whitelist_word_title'),
			'company.description.required' => trans('validation.required'),
			'company.description.mb_between' => trans('validation.mb_between'),
			'company.description.whitelist_word' => trans('validation.whitelist_word'),
			'description.required' => trans('validation.job description required'),
			'description.mb_between' => trans('validation.job description mb_between'),
			'description.whitelist_word' => trans('validation.job description whitelist_word'),
			'start_date.after' => trans('validation.previus_date_validation'),
