...
class LanguageObserver
{
    ...
    public function updating(Language $language)
    {
        ...
        if (Request::filled('default')) {
            if (Request::input('default') == 1 || Request::input('default') == 'on') {
                // Update translated entries
                $this->updateTranslatedEntries(Request::input('abbr'));
            
                // Set default language
                $this->setDefaultLanguage(Request::input('abbr'));
            }
        }
    }
	...
	private function updateDefaultAppLocale($locale)
	{
		...
		DotenvEditor::setKey('APP_FALLBACK_LOCALE', $locale);
		DotenvEditor::save();
	}
